...
data <- readRDS(paste0(rsdir,"objects/data.harmony.Clusterized.rds"))

dataM <- subset(data, subset = Cluster == "Ly6c|Ms4ac Monocytes")



png(paste0(outdir,"/Subclustering.Monocytes2/umap1.png"), width=1800, height=800)
...
png(paste0(outdir,"/Subclustering.Monocytes2/umap.subC1.png"), width=1800, height=800)
DimPlot(dataM, reduction = "umap") 
dev.off()


png(paste0(outdir,"/Subclustering.Monocytes2/Umap_Markers.IFN.png"), width=1400, height=3000)
FeaturePlot_scCustom(dataM, features= c("Ifit3","Ifit1", 
"Ifit2", "Ly6c2", "Spp1","Adgre4", "Gpnmb", "Nrg1"), reduction = "umap",
...
pdf(paste0(outdir,"/Subclustering.Monocytes2/Umap_Markers.pdf"), width=14, height=30)
...
png(paste0(outdir,"/Subclustering.Monocytes2/Umap_SingleR.Image.Fine.png"), width=2200, height=2400)
...
pdf(paste0(outdir,"/Subclustering.Monocytes2/dotplot.pdf"), width=14, height=14)
DotPlot(dataM, features = c("Ly6c2","S100a8","S100a9","Adgre1","Mrc1","C1qa","C1qc","Ifit1","Csfr1","Cxcl9","Ccl5")) + RotatedAxis()
dev.off()

pdf(paste0(outdir,"/Subclustering.Monocytes2/violin.pdf"), width=14, height=14)
...
dataM$Subclustering.Mon [dataM$seurat_clusters %in% c("0")] <- "Nrg1|Cdh1 Macs"
dataM$Subclustering.Mon [dataM$seurat_clusters %in% c("1")] <- "Ly6cHi Monocytes"
dataM$Subclustering.Mon [dataM$seurat_clusters %in% c("2")] <- "Early IFN-TAMs"
dataM$Subclustering.Mon [dataM$seurat_clusters %in% c("3")] <- "Ly6cLo Monocytes"
dataM$Subclustering.Mon [dataM$seurat_clusters %in% c("4")] <- "Early IFN-TAMs"


dataM <- SetIdent(dataM, value = "Subclustering.Mon")
png(paste0(outdir,"/Subclustering.Monocytes2/umap.clusterized.png"), width=1800, height=800)
DimPlot(dataM, reduction = "umap") 
dev.off()
...
write.csv(
  mon.annotation,
  file = paste0(outdir,"/Subclustering.Monocytes2/subclustering_monocytes_annotations.csv"),
  row.names = FALSE
)
